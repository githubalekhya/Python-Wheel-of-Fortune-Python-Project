...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney<VOWEL_COST:
            vowels=[vowel for vowel in VOWELS]
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        sorted_frequency_letters='ZQXJKVBPYGFWMUCLDRHSNIOATE'
        possible_L = self.getPossibleLetters(guessed)
        if len(possible_L) == 0:
            return "pass"
        if self.smartCoinFlip() == True:
            #sorted_frequency_letters=[character for character in self.SORTED_FREQUENCIES]
            return random.choice(sorted_frequency_letters[(len(sorted_frequency_letters)//2)+5:])
        else:
            return random.choice(sorted_frequency_letters)
